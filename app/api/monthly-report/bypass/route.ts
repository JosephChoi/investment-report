  ...
  try {
    ...
    if (!token) {
      ...
      try {
        ...
        if (error) {
          console.error('월간리포트 데이터 가져오기 오류:', error);
          console.error('오류 세부정보:', JSON.stringify(error));
          return errorResponse(
            500, 
            '데이터베이스에서 월간 리포트를 가져오는 중 오류가 발생했습니다.',
            error.message,
            error.code
          );
        }
        
        console.log('월간리포트 데이터 조회 성공 (관리자 권한):', data ? `${data.length}개 항목` : '데이터 없음');
    ...
    try {
      ...
      console.log('월간리포트 데이터 조회 성공:', data ? `${data.length}개 항목` : '데이터 없음');
      
      return successResponse(data || []);
    } catch (dbError: any) {
      console.error('월간리포트 데이터베이스 처리 오류:', dbError);
      return errorResponse(
        500,
        '데이터베이스 처리 중 예외가 발생했습니다.',
        dbError.message || '알 수 없는 데이터베이스 오류',
    ...
    console.error('월간리포트 우회 API 오류:', error);
